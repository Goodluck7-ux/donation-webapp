...
function SidebarContent({
    ...
    const items =
        NAV_BY_ROLE[profile?.role ?? 'DONOR'] ??
        NAV_BY_ROLE.DONOR;
            ...
            <div>
                ...
                <div className="mt-10 rounded-2xl border border-white/[0.07] bg-white/[0.035] p-4">
                    ...
                    <p className="mt-2 text-xs leading-5 text-[#D3E6D8]/55">
                        Every contribution helps move a meaningful cause
                        forward.
                    </p>
                </div>
                ...
                <nav className="mt-9 space-y-1.5">
                    <p className="mb-3 px-3 text-[10px] font-semibold uppercase tracking-[0.16em] text-[#B7E4C7]/35">
                        Workspace
                    </p>
                    ...
                    {items.map((item) => {
                        ...
                        return (
                            <Link
                                ...
                                href={item.href}
                                onClick={onNavigate}
                                className={`group relative flex items-center gap-3 rounded-xl px-3.5 py-3 text-sm transition-all ${active
                                        ? 'bg-white/[0.10] font-semibold text-white shadow-[0_8px_20px_rgba(0,0,0,0.08)]'
                                        : 'font-medium text-[#D3E6D8]/55 hover:bg-white/[0.055] hover:text-white'
                                    }`}
                            >
                                {/* active indicator */}
                                {active && (
                                ...
                                <span
                                    className={`flex h-8 w-8 items-center justify-center rounded-lg transition-colors ${active
                                            ? 'bg-[#B7E4C7]/10 text-[#B7E4C7]'
                                            : 'text-[#B7E4C7]/45 group-hover:text-[#B7E4C7]'
                                        }`}
                                >
                                    <Icon
                                        size={17}
                                ...
                                {active && (
                                    <ArrowUpRight
                                        size={14}
                                        className="ml-auto text-[#B7E4C7]/60"
                                    />
                                )}
                            </Link>
                        );
            ...
            <div className="border-t border-white/[0.08] pt-5">
                <div className="flex items-center gap-3 px-1">
                    ...
                    <div className="min-w-0">
                        ...
                        <p className="mt-0.5 truncate text-[11px] text-[#B7E4C7]/40">
                ...
                <p className="mt-4 px-1 text-[10px] text-[#B7E4C7]/25">
                    © 2026 Finovia Giving
                </p>
            </div>
        </div>
...
export function DashboardShell({
    ...
    return (
        <div className="min-h-screen bg-[#FBF9F3]">
            {/* DESKTOP SIDEBAR */}
            <aside className="fixed inset-y-0 left-0 z-40 hidden w-[272px] flex-col bg-[#0B2418] p-6 md:flex">
                {/* ambient light */}
            ...
            {/* MOBILE TOP BAR */}
            <header className="fixed left-0 right-0 top-0 z-40 flex h-[68px] items-center justify-between border-b border-white/[0.06] bg-[#0B2418] px-5 md:hidden">
                <Link
                    href="/"
                ...
                {mobileOpen && (
                    <div className="fixed inset-0 z-50 md:hidden">
                        ...
                        <motion.aside
                            ...
                            className="relative flex h-full w-[290px] flex-col bg-[#0B2418] p-6 shadow-2xl"
                        >
                            <button
                                onClick={() =>
                                    setMobileOpen(false)
