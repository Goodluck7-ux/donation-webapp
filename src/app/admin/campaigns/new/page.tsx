...
export default function NewCampaignPage() {
    ...
    const busy = uploading || createCampaign.isPending;

    return (
        <div className="p-6 sm:p-10 max-w-2xl mx-auto space-y-8">
            <div>
                <h1 className="font-display text-2xl sm:text-3xl text-text-primary">New campaign</h1>
                <p className="text-text-secondary mt-1 text-sm sm:text-base">Create a new fundraising campaign.</p>
            </div>

            <form onSubmit={handleSubmit} className="space-y-5">
                <div className="space-y-1.5">
                    <label className="text-sm font-medium text-text-primary">Campaign image</label>
                    <label className="block border-2 border-dashed border-border rounded-xl aspect-[16/7] cursor-pointer overflow-hidden hover:border-border-hover transition-colors">
                        {preview ? (
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                        <label className="text-sm font-medium text-text-primary">Goal amount (₦)</label>
                        <input
                            type="number"
                            min={1}
                            required
                ...
                >
                    {uploading ? 'Uploading image…' : createCampaign.isPending ? 'Creating…' : 'Create campaign'}
                </button>
            </form>
        </div>
    );
